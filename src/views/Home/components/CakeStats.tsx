...
const CakeStats = () => {
  ...
  const BengalsPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = BengalsPrice.times(circSupply)

  let BengalsPerBlock = 0
  if (farms && farms[0] && farms[0].BengalsPerBlock) {
    BengalsPerBlock = new BigNumber(farms[0].BengalsPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
  ...
  return (
    <StyledCakeStats>
      <CardBody>
        ...
        <Row>
          <Text fontSize="14px">New Bengals/block</Text>
